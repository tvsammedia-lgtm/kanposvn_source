...
class ParkingReportsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ParkingReportsScreen> createState() => _ParkingReportsScreenState();
}

class _ParkingReportsScreenState extends ConsumerState<ParkingReportsScreen> {
  ...
  void initState() {
    super.initState();
    _selectedCode = ParkingReportService.reportList.first.code;
  }
  ...
  Future<void> _generate() async {
    ...
    try {
      final isarService = ref.read(parkingIsarServiceProvider);
      final isar = await isarService.db;
      final svc = ParkingReportService(Future.value(isar));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Báo Cao Chung'),
  ...
  Widget _buildLeftPanel() {
    final useDateRange = ParkingReportService.reportList
        .firstWhere((r) => r.code == _selectedCode, orElse: () => ParkingReportService.reportList.first)
        .useDateRange;

    final grouped = <String, List<({String code, String title, bool useDateRange})>>{};
    for (final r in ParkingReportService.reportList) {
        ...
        Expanded(child: ListView(
          ...
          children: grouped.entries.expand((entry) => [
            ...
            ...entry.value.map((r) => ListTile(
              ...
              leading: Icon(Icons.description, size: 18, color: _selectedCode == r.code ? const Color(0xFF0F766E) : Colors.grey),
              title: Text(r.title, style: TextStyle(
                fontSize: 13,
                color: _selectedCode == r.code ? const Color(0xFF0F766E) : Colors.black87,
