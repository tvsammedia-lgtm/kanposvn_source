...
class _RestaurantSettingsScreenState
  ...
  void initState() {
    super.initState();
    final s = ref.read(restaurantEinvoiceSettingsProvider).settings;
  ...
  Future<void> _saveEinvoice() async {
    ...
    final store = ref.read(restaurantEinvoiceSettingsProvider);
    final number = int.tryParse(_invoiceNumber.text.trim()) ?? 1;
    final settings = RestaurantEinvoiceSettings(
  ...
  Widget build(BuildContext context) {
    ...
    final einvoiceStore = ref.watch(restaurantEinvoiceSettingsProvider);
