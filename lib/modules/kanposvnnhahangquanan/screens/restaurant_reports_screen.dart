...
/// sơ đồ tầng bàn, danh mục món ăn, giá món ăn.
class RestaurantReportsScreen extends ConsumerStatefulWidget {
  const RestaurantReportsScreen({super.key});

  @override
  ConsumerState<RestaurantReportsScreen> createState() => _RestaurantReportsScreenState();
}

enum _ReportGroup { cash, restaurant }
...
class _RestaurantReportsScreenState extends ConsumerState<RestaurantReportsScreen> {
  static const _reports = [
    ...
    _ReportDescriptor('menu_list', 'Danh mục món ăn',
        Icons.restaurant_menu, _ReportGroup.restaurant),
    _ReportDescriptor('menu_price', 'Giá món ăn',
        Icons.sell, _ReportGroup.restaurant),
  ];

  static const _groupNames = {
    _ReportGroup.cash: 'Tiền mặt',
    _ReportGroup.restaurant: 'Nhà hàng - Quán ăn',
  };
  ...
  Future<void> _runReport() async {
    ...
    try {
      final service = RestaurantReportService(ref.read(restaurantIsarServiceProvider));
      final from = _from;
      final to = _to;
      ...
      switch (_selectedId) {
        ...
        case 'floor':
          ...
          break;
        case 'menu_list':
          result = await service.buildMenuCategoryReport();
          break;
        case 'menu_price':
          result = await service.buildMenuPriceReport();
          break;
        default:
          result = await service.buildCashFundSummary(from: from, to: to);
      }
