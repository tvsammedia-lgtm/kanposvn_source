...
import '../services/gym_report_service.dart';
...
class GymReportsScreen extends ConsumerStatefulWidget {
  const GymReportsScreen({super.key});

  @override
  ConsumerState<GymReportsScreen> createState() => _GymReportsScreenState();
}

class _GymReportsScreenState extends ConsumerState<GymReportsScreen> {
  ...
  void initState() {
    super.initState();
    _selectedCode = GymReportService.reportList.first.code;
  }
  ...
  Future<void> _generate() async {
    ...
    try {
      final isarSvc = ref.read(gymIsarServiceProvider);
      final svc = GymReportService(isarSvc);
  ...
  Widget _buildLeftPanel() {
    final useDateRange = GymReportService.reportList
        .firstWhere((r) => r.code == _selectedCode, orElse: () => GymReportService.reportList.first)
        .useDateRange;

    final grouped = <String, List<({String code, String title, bool useDateRange})>>{};
    for (final r in GymReportService.reportList) {
        ...
        Expanded(child: ListView(
          ...
          children: grouped.entries.expand((entry) => [
            ...
            ...entry.value.map((r) => ListTile(
              ...
              leading: Icon(Icons.description, size: 18, color: _selectedCode == r.code ? const Color(0xFF1E3A8A) : Colors.grey),
              title: Text(r.title, style: TextStyle(
                fontSize: 13,
                color: _selectedCode == r.code ? const Color(0xFF1E3A8A) : Colors.black87,
