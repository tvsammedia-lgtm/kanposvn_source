...
class NgoaiNguReportsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<NgoaiNguReportsScreen> createState() => _NgoaiNguReportsScreenState();
}

class _NgoaiNguReportsScreenState extends ConsumerState<NgoaiNguReportsScreen> {
  ...
  void initState() {
    super.initState();
    _selectedCode = NgoaiNguReportService.reportList.first.code;
  }
  ...
  Future<void> _generate() async {
    ...
    try {
      final isar = ref.read(ngoaiNguIsarProvider);
      final svc = NgoaiNguReportService(Future.value(isar));
  ...
  Widget _buildLeftPanel() {
    final useDateRange = NgoaiNguReportService.reportList
        .firstWhere((r) => r.code == _selectedCode, orElse: () => NgoaiNguReportService.reportList.first)
        .useDateRange;

    final grouped = <String, List<({String code, String title, bool useDateRange})>>{};
    for (final r in NgoaiNguReportService.reportList) {
        ...
        Expanded(child: ListView(
          ...
          children: grouped.entries.expand((entry) => [
            ...
            ...entry.value.map((r) => ListTile(
              ...
              leading: Icon(Icons.description, size: 18, color: _selectedCode == r.code ? const Color(0xFF4F46E5) : Colors.grey),
              title: Text(r.title, style: TextStyle(
                fontSize: 13,
                color: _selectedCode == r.code ? const Color(0xFF4F46E5) : Colors.black87,
