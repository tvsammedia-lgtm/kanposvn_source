...
class BdsReportsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<BdsReportsScreen> createState() => _BdsReportsScreenState();
}

class _BdsReportsScreenState extends ConsumerState<BdsReportsScreen> {
  ...
  void initState() {
    super.initState();
    _selectedCode = BdsReportService.reportList.first.code;
  }
  ...
  Future<void> _generate() async {
    ...
    try {
      ...
      final svc = BdsReportService(isar);
  ...
  Widget _buildLeftPanel() {
    final useDateRange = BdsReportService.reportList
        .firstWhere((r) => r.code == _selectedCode, orElse: () => BdsReportService.reportList.first)
        .useDateRange;

    final grouped = <String, List<({String code, String title, bool useDateRange})>>{};
    for (final r in BdsReportService.reportList) {
        ...
        Expanded(child: ListView(
          ...
          children: grouped.entries.expand((entry) => [
            ...
            ...entry.value.map((r) => ListTile(
              ...
              leading: Icon(Icons.description, size: 18, color: _selectedCode == r.code ? const Color(0xFF0284C7) : Colors.grey),
              title: Text(r.title, style: TextStyle(
                fontSize: 13,
                color: _selectedCode == r.code ? const Color(0xFF0284C7) : Colors.black87,
