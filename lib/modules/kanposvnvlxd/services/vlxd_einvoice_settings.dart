...
class VlxdEinvoiceSettings {
  ...
  factory VlxdEinvoiceSettings.fromJson(Map<String, dynamic> json) {
    return VlxdEinvoiceSettings(
...
class VlxdEinvoiceSettingsStore extends ChangeNotifier {
  static const _kPrefKey = 'vlxd_einvoice_settings_v1';

  VlxdEinvoiceSettings _settings = const VlxdEinvoiceSettings();
  VlxdEinvoiceSettings get settings => _settings;
    ...
    try {
      ...
      if (raw != null && raw.isNotEmpty) {
        ...
        if (decoded is Map<String, dynamic>) {
          _settings = VlxdEinvoiceSettings.fromJson(decoded);
          notifyListeners();
        }
      }
    } catch (_) {}
  ...
  Future<void> update(VlxdEinvoiceSettings settings) async {
    _settings = settings;
    notifyListeners();
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<VlxdEinvoiceSettings> loadOnce() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_kPrefKey);
      if (raw != null && raw.isNotEmpty) {
        final decoded = jsonDecode(raw);
        if (decoded is Map<String, dynamic>) {
          return VlxdEinvoiceSettings.fromJson(decoded);
        }
      }
    } catch (_) {}
    return const VlxdEinvoiceSettings();
  }
}
