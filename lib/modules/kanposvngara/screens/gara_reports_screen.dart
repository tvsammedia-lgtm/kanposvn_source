...
import '../services/gara_report_service.dart';

/// Tab "Báo Cáo" — mô phỏng tất cả 20 báo cáo Crystal (.rpt) từ
/// KANCAFE_VS2022_DEMO cho module Garage Ô Tô.
class GaraReportsScreen extends ConsumerStatefulWidget {
  const GaraReportsScreen({super.key});

  @override
  ConsumerState<GaraReportsScreen> createState() => _GaraReportsScreenState();
}

enum _ReportGroup { cash, inventory, debt, finance, bill, payment }
...
class _GaraReportsScreenState extends ConsumerState<GaraReportsScreen> {
  static const _reports = [
    ...
    _ReportDescriptor('output_by_shift', 'Xuất kho theo phiếu SC', 'rpOutputFromShift', Icons.output, _ReportGroup.inventory),
    _ReportDescriptor('output_by_stock', 'Xuất kho theo phụ tùng', 'rpOutputByStock', Icons.warehouse, _ReportGroup.inventory),
    _ReportDescriptor('shrinkage', 'Hao hụt phụ tùng / vật tư', 'rpBaoCaoHaoHut', Icons.warning_amber, _ReportGroup.inventory),
    ...
    // ── HÓA ĐƠN ──
    _ReportDescriptor('sales_bills', 'Hóa đơn sửa chữa', 'rpBill / rpBillForTax', Icons.receipt_long, _ReportGroup.bill),
    _ReportDescriptor('discount_on_bill', 'Giảm giá trên hóa đơn', 'rpBill_DiscountOnBill', Icons.discount, _ReportGroup.bill),
    // ── BIÊN LAI / THANH TOÁN ──
    _ReportDescriptor('receipts', 'Phiếu thu tiền', 'rpReceipt', Icons.payments, _ReportGroup.payment),
    _ReportDescriptor('payment_by_id', 'Chi tiền theo mã NCC', 'rpPaymentByPaymentID', Icons.money_off, _ReportGroup.payment),
    _ReportDescriptor('payment_cashier', 'Thu tiền cho thu ngân', 'rpPaymentForCashier', Icons.point_of_sale, _ReportGroup.payment),
  ];

  static const _groupNames = {
    _ReportGroup.cash: 'Tiền mặt (rp1111)',
    _ReportGroup.inventory: 'Kho hàng',
    _ReportGroup.debt: 'Công nợ',
    _ReportGroup.finance: 'Tài chính',
    _ReportGroup.bill: 'Hóa đơn / Biên lai',
    _ReportGroup.payment: 'Thanh toán NCC',
  };
  ...
  static const _groupIcons = {
    ...
    _ReportGroup.payment: Icons.money,
  };
  ...
  Future<void> _runReport() async {
    ...
    try {
      final service = GaraReportService(ref.read(garaIsarServiceProvider));
      final from = _from;
      final to = _to;
      ...
      switch (_selectedId) {
        ...
        case 'inventory':
          ...
          break;
        case 'output_by_shift':
          result = await service.buildOutputByShiftReport(from: from, to: to);
          break;
        case 'output_by_stock':
          result = await service.buildOutputByStockReport(from: from, to: to);
          break;
        case 'shrinkage':
          result = await service.buildShrinkageReport(from: from, to: to);
          break;
        case 'import_stock':
          result = await service.buildImportReport(from: from, to: to);
          break;
        case 'customer_debt':
          result = await service.buildCustomerDebtReport();
          break;
        case 'supplier_debt_summary':
          result = await service.buildSupplierDebtSummaryReport();
          break;
        case 'supplier_debt_detail':
          result = await service.buildSupplierDebtDetailReport(from: from, to: to);
          break;
        case 'profit_loss':
        ...
        case 'sales_bills':
          ...
          break;
        case 'discount_on_bill':
          result = await service.buildDiscountOnBillReport(from: from, to: to);
          break;
        case 'receipts':
          result = await service.buildReceiptReport(from: from, to: to);
          break;
        case 'payment_by_id':
          result = await service.buildPaymentByPaymentIdReport(from: from, to: to);
          break;
        case 'payment_cashier':
          result = await service.buildPaymentForCashierReport(from: from, to: to);
          break;
        default:
          result = await service.buildCashFundSummary(from: from, to: to);
      }
      ...
      child: Row(
        children: [
          if (needsDate) ...[
            ...
            const SizedBox(width: 8),
            Text(
              'Báo cáo tổng hợp — không cần chọn ngày',
    ...
    return [
      ...
      for (final r in items)
        ListTile(
          ...
          subtitle: r.rptCode.isNotEmpty
              ? Text(r.rptCode, style: TextStyle(fontSize: 10, color: theme.colorScheme.outline))
              : null,
