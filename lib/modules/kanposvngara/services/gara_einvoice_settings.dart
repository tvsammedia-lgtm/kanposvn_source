...
class GaraEinvoiceSettings {
  ...
  factory GaraEinvoiceSettings.fromJson(Map<String, dynamic> json) {
    return GaraEinvoiceSettings(
...
class GaraEinvoiceSettingsStore extends ChangeNotifier {
  static const _kPrefKey = 'gara_einvoice_settings_v1';

  GaraEinvoiceSettings _settings = const GaraEinvoiceSettings();
  GaraEinvoiceSettings get settings => _settings;
    ...
    try {
      ...
      if (raw != null && raw.isNotEmpty) {
        ...
        if (decoded is Map<String, dynamic>) {
          _settings = GaraEinvoiceSettings.fromJson(decoded);
          notifyListeners();
        }
      }
    } catch (_) {}
  ...
  Future<void> update(GaraEinvoiceSettings settings) async {
    _settings = settings;
    notifyListeners();
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<GaraEinvoiceSettings> loadOnce() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_kPrefKey);
      if (raw != null && raw.isNotEmpty) {
        final decoded = jsonDecode(raw);
        if (decoded is Map<String, dynamic>) {
          return GaraEinvoiceSettings.fromJson(decoded);
        }
      }
    } catch (_) {}
    return const GaraEinvoiceSettings();
  }
}
