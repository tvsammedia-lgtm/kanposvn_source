...
import '../services/nhatro_report_service.dart';
...
class NhatroReportsScreen extends ConsumerStatefulWidget {
  const NhatroReportsScreen({super.key});

  @override
  ConsumerState<NhatroReportsScreen> createState() => _NhatroReportsScreenState();
}

class _NhatroReportsScreenState extends ConsumerState<NhatroReportsScreen> {
  ...
  void initState() {
    super.initState();
    _selectedCode = NhatroReportService.reportList.first.code;
  }
  ...
  Future<void> _generate() async {
    ...
    try {
      ...
      final svc = NhatroReportService(isar);
  ...
  Widget _buildLeftPanel() {
    final useDateRange = NhatroReportService.reportList
        .firstWhere((r) => r.code == _selectedCode, orElse: () => NhatroReportService.reportList.first)
        .useDateRange;

    final grouped = <String, List<({String code, String title, bool useDateRange})>>{};
    for (final r in NhatroReportService.reportList) {
        ...
        Expanded(child: ListView(
          ...
          children: grouped.entries.expand((entry) => [
            ...
            ...entry.value.map((r) => ListTile(
              ...
              leading: Icon(Icons.description, size: 18, color: _selectedCode == r.code ? const Color(0xFF475569) : Colors.grey),
              title: Text(r.title, style: TextStyle(
                fontSize: 13,
                color: _selectedCode == r.code ? const Color(0xFF475569) : Colors.black87,
