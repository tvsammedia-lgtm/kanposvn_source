...
/// KANCAFE_VS2022_DEMO cho module Spa.
class SpaReportsScreen extends ConsumerStatefulWidget {
  const SpaReportsScreen({super.key});

  @override
  ConsumerState<SpaReportsScreen> createState() => _SpaReportsScreenState();
}

enum _ReportGroup { cash, inventory, debt, finance, bill, technician, extra }
...
class _SpaReportsScreenState extends ConsumerState<SpaReportsScreen> {
  static const _reports = [
    ...
    _ReportDescriptor('import_stock', 'Nhập kho dược liệu', 'rpInput / rpListInput', Icons.input, _ReportGroup.inventory),
    _ReportDescriptor('output_by_shift', 'Xuất kho theo buổi DV', 'rpOutputFromShift', Icons.output, _ReportGroup.inventory),
    _ReportDescriptor('output_by_stock', 'Xuất kho theo dược liệu', 'rpOutputByStock', Icons.warehouse, _ReportGroup.inventory),
    _ReportDescriptor('shrinkage', 'Hao hụt dược liệu', 'rpBaoCaoHaoHut', Icons.warning_amber, _ReportGroup.inventory),
    ...
    _ReportDescriptor('discount_on_bill', 'Combo / Gói dịch vụ', 'rpBill_DiscountOnBill', Icons.card_giftcard, _ReportGroup.bill),
    _ReportDescriptor('receipts', 'Phiếu thu tiền', 'rpReceipt', Icons.payments, _ReportGroup.bill),
    _ReportDescriptor('revenue_by_tech', 'Doanh thu theo KTV', 'rpBillForField', Icons.badge, _ReportGroup.technician),
    _ReportDescriptor('revenue_by_service', 'Doanh thu theo dịch vụ', 'rpBillForField_IsNotUsed', Icons.spa, _ReportGroup.technician),
    _ReportDescriptor('discount_on_item', 'Chi tiết buổi dịch vụ', 'rpBill_DiscountOnFoodOrDrink', Icons.event_available, _ReportGroup.extra),
    _ReportDescriptor('payment_by_id', 'Chi tiêu theo mã', 'rpPaymentByPaymentID', Icons.money_off, _ReportGroup.extra),
  ...
  static const _groupNames = {
    ...
    _ReportGroup.technician: 'Doanh thu theo KTV / DV',
  ...
  static const _groupIcons = {
    ...
    _ReportGroup.technician: Icons.badge,
  ...
  Future<void> _runReport() async {
    ...
    try {
      final isar = await ref.read(spaIsarServiceProvider).db;
      final service = SpaReportService(isar);
      final from = _from;
      final to = _to;
      ...
      switch (_selectedId) {
        ...
        case 'revenue_by_tech': result = await service.buildRevenueByTechnicianReport(from: from, to: to); break;
