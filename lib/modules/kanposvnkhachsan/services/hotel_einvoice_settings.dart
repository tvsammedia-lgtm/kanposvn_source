...
class HotelEinvoiceSettings {
  ...
  HotelEinvoiceSettings copyWith({
    String? companyName,
    String? taxCode,
    String? address,
    String? phone,
  ...
  factory HotelEinvoiceSettings.fromJson(Map<String, dynamic> json) {
    return HotelEinvoiceSettings(
...
class HotelEinvoiceSettingsStore extends ChangeNotifier {
  static const _kPrefKey = 'hotel_einvoice_settings_v1';

  HotelEinvoiceSettings _settings = const HotelEinvoiceSettings();
  HotelEinvoiceSettings get settings => _settings;
    ...
    try {
      ...
      if (raw != null && raw.isNotEmpty) {
        ...
        if (decoded is Map<String, dynamic>) {
          _settings = HotelEinvoiceSettings.fromJson(decoded);
          notifyListeners();
        }
      }
    } catch (_) {}
  ...
  Future<void> update(HotelEinvoiceSettings settings) async {
    _settings = settings;
    notifyListeners();
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<HotelEinvoiceSettings> loadOnce() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_kPrefKey);
      if (raw != null && raw.isNotEmpty) {
        final decoded = jsonDecode(raw);
        if (decoded is Map<String, dynamic>) {
          return HotelEinvoiceSettings.fromJson(decoded);
        }
      }
    } catch (_) {}
    return const HotelEinvoiceSettings();
  }
}
