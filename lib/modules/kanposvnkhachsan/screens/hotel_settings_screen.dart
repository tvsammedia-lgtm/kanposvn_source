...
class _HotelSettingsScreenState extends ConsumerState<HotelSettingsScreen> {
  ...
  void initState() {
    super.initState();
    final s = ref.read(hotelEinvoiceSettingsProvider).settings;
  ...
  Future<void> _saveEinvoice() async {
    ...
    final store = ref.read(hotelEinvoiceSettingsProvider);
    final number = int.tryParse(_invoiceNumber.text.trim()) ?? 1;
    final settings = HotelEinvoiceSettings(
    ...
    HotelReportService.configureCompany(
  ...
  Widget build(BuildContext context) {
    final einvoiceStore = ref.watch(hotelEinvoiceSettingsProvider);
