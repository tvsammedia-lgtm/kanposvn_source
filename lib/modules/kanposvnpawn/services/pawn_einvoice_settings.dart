...
class PawnEinvoiceSettings {
  ...
  factory PawnEinvoiceSettings.fromJson(Map<String, dynamic> json) {
    return PawnEinvoiceSettings(
...
class PawnEinvoiceSettingsStore extends ChangeNotifier {
  static const _kPrefKey = 'pawn_einvoice_settings_v1';

  PawnEinvoiceSettings _settings = const PawnEinvoiceSettings();
  PawnEinvoiceSettings get settings => _settings;
    ...
    try {
      ...
      if (raw != null && raw.isNotEmpty) {
        ...
        if (decoded is Map<String, dynamic>) {
          _settings = PawnEinvoiceSettings.fromJson(decoded);
          notifyListeners();
        }
      }
    } catch (_) {}
  ...
  Future<void> update(PawnEinvoiceSettings settings) async {
    _settings = settings;
    notifyListeners();
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<PawnEinvoiceSettings> loadOnce() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_kPrefKey);
      if (raw != null && raw.isNotEmpty) {
        final decoded = jsonDecode(raw);
        if (decoded is Map<String, dynamic>) {
          return PawnEinvoiceSettings.fromJson(decoded);
        }
      }
    } catch (_) {}
    return const PawnEinvoiceSettings();
  }
}
