...
class TapHoaEinvoiceSettings {
  ...
  TapHoaEinvoiceSettings copyWith({
    String? companyName,
    String? taxCode,
    String? address,
    String? phone,
  ...
  factory TapHoaEinvoiceSettings.fromJson(Map<String, dynamic> json) {
    return TapHoaEinvoiceSettings(
...
class TapHoaEinvoiceSettingsStore extends ChangeNotifier {
  static const _kPrefKey = 'taphoa_einvoice_settings_v1';

  TapHoaEinvoiceSettings _settings = const TapHoaEinvoiceSettings();
  TapHoaEinvoiceSettings get settings => _settings;
    ...
    try {
      ...
      if (raw != null && raw.isNotEmpty) {
        ...
        if (decoded is Map<String, dynamic>) {
          _settings = TapHoaEinvoiceSettings.fromJson(decoded);
          notifyListeners();
        }
      }
    } catch (_) {}
  ...
  Future<void> update(TapHoaEinvoiceSettings settings) async {
    _settings = settings;
    notifyListeners();
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<TapHoaEinvoiceSettings> loadOnce() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_kPrefKey);
      if (raw != null && raw.isNotEmpty) {
        final decoded = jsonDecode(raw);
        if (decoded is Map<String, dynamic>) {
          return TapHoaEinvoiceSettings.fromJson(decoded);
        }
      }
    } catch (_) {}
    return const TapHoaEinvoiceSettings();
  }
}
