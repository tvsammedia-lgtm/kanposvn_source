...
class BarberEinvoiceSettings {
  ...
  BarberEinvoiceSettings copyWith({
    String? companyName,
    String? taxCode,
    String? address,
    String? phone,
  ...
  factory BarberEinvoiceSettings.fromJson(Map<String, dynamic> json) {
    return BarberEinvoiceSettings(
...
class BarberEinvoiceSettingsStore extends ChangeNotifier {
  static const _kPrefKey = 'barber_einvoice_settings_v1';

  BarberEinvoiceSettings _settings = const BarberEinvoiceSettings();
  BarberEinvoiceSettings get settings => _settings;
    ...
    try {
      ...
      if (raw != null && raw.isNotEmpty) {
        ...
        if (decoded is Map<String, dynamic>) {
          _settings = BarberEinvoiceSettings.fromJson(decoded);
          notifyListeners();
        }
      }
    } catch (_) {}
  ...
  Future<void> update(BarberEinvoiceSettings settings) async {
    _settings = settings;
    notifyListeners();
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<BarberEinvoiceSettings> loadOnce() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_kPrefKey);
      if (raw != null && raw.isNotEmpty) {
        final decoded = jsonDecode(raw);
        if (decoded is Map<String, dynamic>) {
          return BarberEinvoiceSettings.fromJson(decoded);
        }
      }
    } catch (_) {}
    return const BarberEinvoiceSettings();
  }
}
