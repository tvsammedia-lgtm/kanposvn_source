...
class CafeEinvoiceSettings {
  ...
  factory CafeEinvoiceSettings.fromJson(Map<String, dynamic> json) {
    return CafeEinvoiceSettings(
...
class CafeEinvoiceSettingsStore extends ChangeNotifier {
  static const _kPrefKey = 'cafe_einvoice_settings_v1';

  CafeEinvoiceSettings _settings = const CafeEinvoiceSettings();
  CafeEinvoiceSettings get settings => _settings;
    ...
    try {
      ...
      if (raw != null && raw.isNotEmpty) {
        ...
        if (decoded is Map<String, dynamic>) {
          _settings = CafeEinvoiceSettings.fromJson(decoded);
          notifyListeners();
        }
      }
    } catch (_) {}
  ...
  Future<void> update(CafeEinvoiceSettings settings) async {
    _settings = settings;
    notifyListeners();
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<CafeEinvoiceSettings> loadOnce() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_kPrefKey);
      if (raw != null && raw.isNotEmpty) {
        final decoded = jsonDecode(raw);
        if (decoded is Map<String, dynamic>) {
          return CafeEinvoiceSettings.fromJson(decoded);
        }
      }
    } catch (_) {}
    return const CafeEinvoiceSettings();
  }
}
