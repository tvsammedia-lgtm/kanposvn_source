...
Future<void> printBillPdf(CafeOrder cart) async {
  pw.Font? font;
  pw.Font? fontBold;
  try {
  ...
  pdf.addPage(
    pw.Page(
      ...
      build: (ctx) => pw.Column(
        ...
        children: [
          pw.Center(child: pw.Text('KANPOSVN', style: pw.TextStyle(font: fontBold, fontSize: 14, fontWeight: pw.FontWeight.bold))),
          ...
          pw.Text('Mã đơn: ${cart.orderCode}', style: pw.TextStyle(font: fontBold, fontSize: 9, fontWeight: pw.FontWeight.bold)),
          pw.Text('Ngày: ${DateFormat('dd/MM/yyyy HH:mm').format(DateTime.now())}', style: pw.TextStyle(fontSize: 9)),
          if (cart.occupiedAt != null)
            pw.Text('Giờ vào: ${DateFormat('dd/MM/yyyy HH:mm').format(cart.occupiedAt!)}', style: pw.TextStyle(fontSize: 9)),
          if (cart.paidAt != null)
            pw.Text('Giờ ra: ${DateFormat('dd/MM/yyyy HH:mm').format(cart.paidAt!)}', style: pw.TextStyle(fontSize: 9)),
          ...
          if (cart.customerName.isNotEmpty && cart.customerName != 'Khách lẻ')
            pw.Text('Khách: ${cart.customerName}', style: pw.TextStyle(fontSize: 9)),
          pw.Text('Thanh toán: ${cart.paymentMethod.label}', style: pw.TextStyle(fontSize: 9)),
          pw.SizedBox(height: 3),
          pw.Center(child: pw.Text('-' * width, style: pw.TextStyle(fontSize: 8))),

          pw.Text(fmtRow('Tên món', 'Thành tiền'), style: pw.TextStyle(font: fontBold, fontSize: 9, fontWeight: pw.FontWeight.bold)),
          pw.Center(child: pw.Text('-' * width, style: pw.TextStyle(fontSize: 8))),
          ...
          ...cart.items.map((item) {
            ...
            return pw.Column(
              crossAxisAlignment: pw.CrossAxisAlignment.start,
              children: [
                pw.Text('${item.menuItemName} (${item.selectedSize.name})', style: pw.TextStyle(font: fontBold, fontSize: 9, fontWeight: pw.FontWeight.bold)),
                if (item.selectedToppings.isNotEmpty)
                  pw.Text('  + ${item.selectedToppings.map((t) => '${t.name} ${_currency.format(t.price)}').join(', ')}', style: pw.TextStyle(fontSize: 8)),
                if (item.note.isNotEmpty)
                  pw.Text('  * ${item.note}', style: pw.TextStyle(fontSize: 8)),
                pw.Text(fmtRow('  ${item.quantity} x ${_currency.format(unitPrice)}', _currency.format(item.totalPrice)), style: pw.TextStyle(fontSize: 9)),
          ...
          pw.Text(fmtRow('Tạm tính:', _currency.format(cart.subtotal)), style: pw.TextStyle(fontSize: 9)),
          if (cart.totalDiscount > 0)
            pw.Text(fmtRow('Giảm giá:', '-${_currency.format(cart.totalDiscount)}'), style: pw.TextStyle(fontSize: 9)),
          pw.SizedBox(height: 3),
          pw.Center(child: pw.Text('=' * width, style: pw.TextStyle(fontSize: 8))),
          pw.Text(fmtRow('TỔNG CỘNG:', _currency.format(cart.grandTotal)),
            style: pw.TextStyle(font: fontBold, fontSize: 11, fontWeight: pw.FontWeight.bold)),
          pw.Center(child: pw.Text('=' * width, style: pw.TextStyle(fontSize: 8))),

          pw.SizedBox(height: 3),
          if (cart.status == OrderStatus.daThanhToan)
            pw.Text('Trạng thái: ĐÃ THANH TOÁN', style: pw.TextStyle(fontSize: 9)),
          pw.Center(child: pw.Text('-' * width, style: pw.TextStyle(fontSize: 8))),
          ...
          pw.Center(child: pw.Text(cart.orderCode, style: pw.TextStyle(font: fontBold, fontSize: 9, fontWeight: pw.FontWeight.bold))),
