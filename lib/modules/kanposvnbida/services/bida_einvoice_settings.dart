...
class BidaEinvoiceSettings {
  ...
  factory BidaEinvoiceSettings.fromJson(Map<String, dynamic> json) {
    return BidaEinvoiceSettings(
...
class BidaEinvoiceSettingsStore extends ChangeNotifier {
  static const _kPrefKey = 'bida_einvoice_settings_v1';

  BidaEinvoiceSettings _settings = const BidaEinvoiceSettings();
  BidaEinvoiceSettings get settings => _settings;
    ...
    try {
      ...
      if (raw != null && raw.isNotEmpty) {
        ...
        if (decoded is Map<String, dynamic>) {
          _settings = BidaEinvoiceSettings.fromJson(decoded);
          notifyListeners();
        }
      }
    } catch (_) {}
  ...
  Future<void> update(BidaEinvoiceSettings settings) async {
    _settings = settings;
    notifyListeners();
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  static Future<BidaEinvoiceSettings> loadOnce() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final raw = prefs.getString(_kPrefKey);
      if (raw != null && raw.isNotEmpty) {
        final decoded = jsonDecode(raw);
        if (decoded is Map<String, dynamic>) {
          return BidaEinvoiceSettings.fromJson(decoded);
        }
      }
    } catch (_) {}
    return const BidaEinvoiceSettings();
  }
}
